...
def benchmark_Pengurangan():
    print("\nBenchmark  Memori Pengurangan Homomorfik (dengan GC Manual)")
    ...
    for size in keys_arr:
        print(f"\n--- Pengurangan Homomorfik 100 data dengan kunci {size}-bits ---")
        peak_memory_arr = []
        
        pub, priv = utils.generate_paillier_keypair(n_length=size)
        saldo = pub.encrypt(sum(data_arr)).ciphertext()
        cipher_arr = []
        ...
        for data in cipher_arr:  
            ...
            subtraction = utils.paillier_subtraction(saldo, data.ciphertext(), pub.n)
