...
class TestKebenaran(SimpleTestCase):
    def test_waktu_penambahan(self):
        ...
        for size in keys_arr:
            print(f"Penambahan 100 data dengan kunci {size}-bits")
            self.PublicKey, self.PrivateKey = utils.generate_paillier_keypair(n_length=size)
            time_arr = []
            jumlah_homomorfik = self.PublicKey.encrypt(0).ciphertext()

            for val in data_arr:
                encrypted_val = self.PublicKey.encrypt(val)
                start_time = time.perf_counter()
                homomorfik = utils.paillier_addition(jumlah_homomorfik, encrypted_val.ciphertext(), self.PublicKey.n)
            ...
            self.assertEqual(decry_data, jumlah_dataarr, "Jumlah dari data tidak sama!")