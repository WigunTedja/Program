...
class TestKebenaran(SimpleTestCase):
    def test_waktu_enkripsi(self):
        ...
        for size in keys_arr:
            print(f"Enkripsi 100 data dengan kunci {size}-bits")
            self.PublicKey, self.PrivateKey = utils.generate_paillier_keypair(n_length=size)
            time_arr = []
            total_time = 0
            for val in data_arr:  
                start_time = time.perf_counter()
                enc_data = self.PublicKey.encrypt(val)
            ...
            print(f"Rata-rata waktu enkripsi dengan kunci {size}-bit: {mean_waktu} detik")